...
never in this repository, and from there into a Kubernetes Secret.
...
# hosted only: forge's praxis-deploy stack always lays down the ollama config
kubectl create configmap praxis-config --from-file=praxis.yaml=configs/openai.yaml \
  --dry-run=client -o yaml | kubectl apply -f -
kubectl rollout restart deployment/praxis-proxy
```

The Deployment marks that Secret optional, so the Ollama path needs none.
...
```bash
ANTHROPIC_BASE_URL=http://localhost:38080 \
...
| gateway | admin | Grafana | Perses | Prometheus |
| --- | --- | --- | --- | --- |
| 38080 | 38901 | 33000 | 33001 | 39090 |
