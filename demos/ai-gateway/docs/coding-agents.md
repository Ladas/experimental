...
Works the same for either target. Set the gateway once and every block below
copies as-is.

```bash
export GATEWAY=http://localhost:8080      # compose
# export GATEWAY=http://localhost:38080   # KIND
```

> **The key in these blocks is a dummy.** Codex and Claude Code refuse to start
> without something in their key variable, and the demo gateway does not check
> it. Your real provider key lives in the gateway's environment, put there by
> `./demo-podman up <provider>` or `./demo-kind up <provider>` from your OS
> keychain, and the agent never holds it.
...
> the burst demo, not for agents. `up ollama` pulls only the small one;
...
`ANTHROPIC_MODEL` matters: without it Claude Code asks for a `claude-*` model,
which the gateway forwards to Ollama, which does not have it.
...
model, which is the more interesting picture — but give each an `agent-daily`
rule first, or they land on the catch-all and get 429'd.
...
OPENAI_API_KEY=dummy codex \
  -c model_provider="praxis-${GATEWAY##*:}" \
...
`wire_api = "responses"` is not a preference. Codex dropped chat-completions in
February 2026 and `wire_api = "chat"` is now a startup error, so a gateway in
front of it has to serve `/v1/responses`. This one does; praxis translates it to
...
## Did it actually reach the gateway?
