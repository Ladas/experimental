...
**`configs/laptop.yaml`** — one person, one machine:

| Rule | Match | Budget |
| --- | --- | --- |
| `agent-daily` | `X-Model: qwen3.8:27b` | 10,000,000 tokens/**day** |
| `default` | catch-all | 20,000 tokens/**min** |

**`configs/<provider>.yaml`** — the demo stack, tuned small so a burst trips it:
...
> copies stripped. `basic_auth` authenticates but publishes no identity to key a
> budget off, so everyone matching a rule still shares it. Tracked at grid#101.
...
## Why a budget only trips under concurrent load
