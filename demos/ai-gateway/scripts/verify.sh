...
if [ "$(chat "${MODEL}" /dev/null 'X-Tier: premium')" = "429" ]; then
  fail "premium tier was rate limited"
fi
pass "premium tier is not rate limited"
fi
...
if [ -n "${TEMPO}" ]; then
  ...
  for _ in $(seq 1 15); do
    if curl --fail --silent --max-time 5 "${TEMPO}/api/search?limit=5" | grep -q '"rootServiceName"'; then
      found=1
      break
    fi
    sleep 2
  done
  [ "${found}" = "1" ] || fail "no traces found in Tempo at ${TEMPO}"
  pass "traces in Tempo"
fi
...
if [ -n "${PROMETHEUS}" ]; then
  ...
  for _ in $(seq 1 15); do
    if curl --fail --silent --max-time 5 --get \
      ...
      "${PROMETHEUS}/api/v1/query" | grep -q '"value"'; then
      up=1
      break
    fi
    sleep 2
  done
  [ "${up}" = "1" ] || fail "Prometheus has no up{job=\"${PRAXIS_JOB}\"} series at ${PROMETHEUS}"
